...
# Data parameters
data_folder = './data'  # folder with data files, you can make a new folder called data in the main directory, already done
...
n_layers = 6  # number of layers in the Encoder and Decoder 6
...
batches_per_step = 2000 // tokens_in_batch  # perform a training step, i.e. update parameters, once every so many batches
print_frequency = 20  # print status once every so many steps
n_steps = 500  # number of training steps
warmup_steps = 400  # number of warmup steps where learning rate is increased linearly;
step = 1  # the step number, start from 1 to prevent math error in the next line
lr = get_lr(step=step, d_model=d_model,
            warmup_steps=warmup_steps)  # see utils.py for learning rate schedule;
...
def train(train_loader, model, criterion, optimizer, epoch, step):
    ...
    for i, (source_sequences, target_sequences, source_sequence_lengths, target_sequence_lengths) in enumerate(
        ...
        if (i + 1) % batches_per_step == 0:
            ...
            if epoch in [epochs - 1, epochs - 2] and step % 500 == 0:  # 'epoch' is 0-indexed
