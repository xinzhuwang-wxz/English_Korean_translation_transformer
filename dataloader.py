...
class SequenceLoader(object):
    ...
    def __init__(self, data_folder, source_suffix, target_suffix, split, tokens_in_batch):
        ...
        if len(source_data) != len(target_data):
            print("There are a different number of source or target sequences !!!!!STOP STOP STOP!!!!! please check the data's length and divide data again")
            exit()
        ...
        self.data = list(zip(source_data, target_data, source_lengths, target_lengths))  # zip->single: [[four elements in all 1st], [four elements in all 2nd]]
    ...
    def create_batches(self):
        ...
        if self.for_training:
            ...
            chunks = [list(g) for _, g in groupby(self.data, key=lambda x: x[3])]  # same target length datas will be grouped to one chunk

            # chunks = [[group1], [[after zip, 4 elements], [], []], ]
            ...
            for chunk in chunks:
                ...
                seqs_per_batch = self.tokens_in_batch // chunk[0][3]  # each chunk has same num of traget token
                # Split chunk into batches
                self.all_batches.extend([chunk[i: i + seqs_per_batch] for i in range(0, len(chunk), seqs_per_batch)])

            # self.all_batches = [[batch_1], [(four elements),(),()]]
            ...
            self.all_batches = [[d] for d in self.data]   # [[(four elements)]],[[batch_2]],[]]
            self.n_batches = len(self.all_batches)
            self.current_batch = -1
    ...
    def __next__(self):
        ...
        self.current_batch += 1
        try:
            source_data, target_data, source_lengths, target_lengths = zip(*self.all_batches[self.current_batch])  # every "source_data" in the batch will combine to the source_data as list
